...
public class ApacheLog implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -3429531061262801993L;
	private byte[] data;

    public ApacheLog(byte[] data) {
        this.data = data;
    }

    public ApacheLog(String data) {
    ...
    public void readObject(java.io.ObjectInputStream in) throws IOException, ClassNotFoundException {
        ...
        int n = in.read(data);
        if (n != length) {
            throw new IOException("Couldn't read entire ApacheLog object, only got " + n + " bytes");
        }
